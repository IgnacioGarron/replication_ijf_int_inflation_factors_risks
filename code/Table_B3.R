...
Table<-data.frame(matrix(nrow=13,ncol = 15))
i<-1
colnames(Table) <-c(rep(c("0.05","0.5","0.95"),5))
rownames(Table)<-c("Intercept","pval1","Ly","pval2","FGlobal","pval3","FRegional","pval4","FIncome",
                   "pval5","R1","AIC","AIC_AR")
for (name in c("Brazil","Mexico","South Africa","China","India")) {
...
kbl(round(Table,3), caption = "Table B3",booktabs = T,
    linesep = c(""),
    format="latex") %>%
  kable_classic(full_width = F) %>% 
  kable_styling(latex_options = "scale_down") %>% 
  save_kable(paste('../output/Tables/Table_B3.tex'),float = FALSE)
